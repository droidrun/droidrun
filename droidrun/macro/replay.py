    ...
    async def replay_action(self, action: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if action_type == "start_app":
                ...
                duration = action.get("duration_ms", 0.3)

                logger.info(
                    f"👆 Swiping from ({start_x}, {start_y}) to ({end_x}, {end_y}) in {duration}seconds"
                )
                result = await tools.swipe(start_x, start_y, end_x, end_y, duration)
                logger.debug(f"   Result: {result}")
                return True
                ...
                result = await tools.drag(start_x, start_y, end_x, end_y, duration)
                logger.debug(f"   Result: {result}")
                return True
